...
## 🚩 5. Outer Architecture 목록
- EC2: "Spring API Gateway, Eureka, Config Server, Jenkins ..
- ECS: 각 도메인별 서비스
- CI/CD :ECR에 푸쉬된 이미지를 기반으로 서비스 업데이트, 블루/그린 배포 방식 사용, 배포 전용 브랜치 PR & Push & Deploy Label -> Webhook, ECS 배포 프로세스 진행, 미리 정의된 Task Definition 사용, S3 버킷에서 환경변수 관리
...
## 🚩 10. 트러블슈팅
- **문제**: 레거시 서비스의 RT 재발급 정책 문제
  - AT(Access Token) 만료 및 RT(Refresh Token) 유효 상황에서 클라이언트가 명시적으로 AT 재발급 요청을 보내지 않아도 서버가 자동으로 RT 검증 후 재발급을 처리하는 구조였습니다.

   - JWT 토큰 검증과 재발급의 책임이 명확하게 분리되지 않았으며, API Gateway의 역할이 모호해졌습니다.
  - 해결법
     - 새로운 엔드포인트 auth/reissue를 추가하여 명시적 요청을 통해 AT를 재발급하도록 구현하였습니다. 클라이언트는 이 엔드포인트를 통해 재발급 요청을 보낼 수 있습니다.
API Gateway가 AT 검증만을 담당하도록 설정하여, 역할을 명확하게 분리하였습니다. AuthService는 토큰 발급만을 담당하도록 변경하여 토큰 재발급 처리 책임을 분리했습니다.

- **문제**: 내부 통신 간 요청 검증 부재
  - Micro Service 간 통신은 폐쇄망에서 이루어지지만, 전체 MSA 인증은 API Gateway에서 전담하고 있습니다.
    
   - 그럼에도 불구하고 내부 통신 간 최소한의 보안 조치가 부족하여, 보안상 우려가 있었습니다.
     
  - 해결법
 
       - 내부 서비스 간 통신을 위한 별도의 Passport 토큰을 발급하여, 각 서비스 간의 요청을 검증할 수 있도록 했습니다.
       - 각 서비스 간의 RequestHeader에 포함된 토큰을 검증하여, 요청의 진위 여부를 확인할 수 있도록 했습니다.
       - Config 서버에서 토큰을 관리하며, 유사 시(토큰 탈취 등)에는 빠르게 토큰을 교체할 수 있도록 하여 보안을 강화했습니다
       - 내부 통신을 처리하는 Controller에 Interceptor를 구현하여, 내부 서비스 간의 요청에 대해 토큰을 검증하고, 유효한 토큰만을 처리하도록 했습니다.

- **문제**: EC2 API Gateway 호출 문제 - ECS 서비스 라우팅 불가
  - 로컬 환경에서는 정상 동작하지만, 외부 요청에서 자원 공유(CORS) 문제가 발생하여 API 호출이 실패하는 상황이었습니다.
    
   - 또한, ECS 서비스에 대한 호출이 API Gateway를 통해 라우팅되지 않는 문제도 발생했습니다. 이 문제는 Eureka 등록 시 Private IP 정보가 누락되어 발생한 것이었습니다.
     
  - 해결법
 
       - 외부 도메인에서의 요청을 허용하기 위해 Global CORS 설정을 추가하여, 외부 요청에서도 자원 공유가 원활하게 이루어지도록 했습니다.
       - Eureka에 서비스가 등록될 때, Private IP 정보를 함께 등록하는 코드를 추가하여, API Gateway가 ECS 서비스로 요청을 정상적으로 라우팅할 수 있도록 했습니다.

## 🚩 11. 향후 개선 여지
- **모니터링 및 로깅**
     분산 추적 시스템을 도입하여 마이크로서비스 간의 요청 흐름을 추적하고, 서비스 간 의존성 문제나 성능 문제를 실시간으로 감지할 수 있도록 할 예정입니다.

또한, 중앙화된 로깅 시스템을 활용해 로그를 통합하여 문제 발생 시 신속하게 대응할 수 있는 체계를 마련할 것입니다.

- **API Rate Limiting**
  API Gateway에서 Rate Limiting 기능을 적용하여 과도한 트래픽을 방지하고, DDoS 공격 및 서비스 남용을 방지할 수 있는 기능을 추가할 계획입니다.
